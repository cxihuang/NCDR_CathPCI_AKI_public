# Data pre-processing
# for contemporary cohort: July 1, 2015 - March 31, 2017
library(lubridate)

folder='/data/Projects/ACC_NCDR/NCDR/CATHPCI/CPV4/CPV4_07142017/csv/'
file_list=list.files(path=folder,pattern = "*.csv")

visit=read.csv(paste(folder,file_list[16],sep=''),header = T,sep = ',')
pepsd=read.csv(paste(folder,file_list[11],sep=''),header = T,sep = ',')
psubm=read.csv(paste(folder,file_list[14],sep=''),header = T,sep = ',')
...
sel=rep(1,length(DCDate.conv))
sel[which(DCDate.year<2015)]=0
sel[which((DCDate.year==2015) & (DCDate.month<7))]=0

in.date=sel&PCI
full.data$in.date=in.date
sub.data=subset(full.data,in.date==1)

sub.data=sub.data[with(sub.data,order(EpisodeKey,ProcedureDate,ProcedureTime)),]
dup=duplicated(sub.data$EpisodeKey)
in.dupEpKey=rep(1,dim(sub.data)[1])
in.dupEpKey[dup]=0 # retain only the first PCI
sub.data$in.dupEpKey=in.dupEpKey# 

in.sdDC=rep(1,dim(sub.data)[1])
in.sdDC[sub.data$DCDate==sub.data$ProcedureDate]=0 
sub.data$in.sdDC=in.sdDC# 
# missing pre- and post- procedure creatinine or cannot be identified by post-dialysis
in.creat=rep(1,dim(sub.data)[1])
in.creat[(is.na(sub.data$PreProcCreat)|is.na(sub.data$PostProcCreat))
         &(sub.data$PostDialysis==0|is.na(sub.data$PostDialysis))]=0
...
# model 1 variables
vars_model1=c("AKI","Age","IABPst","egfrc","Prior2weeksHF","Diabetes",
           "PriorHF","PriorCVD","cadpresent","PriorCardioShock","PriorCardiacArrest","Anemia")

# model 8 variables
vars_model8=c("AKI","Age","AdmtSource","PriorHF", "Diabetescat","CADPresentationcat",
           "Prior2weeksHFcat","PriorCardiacArrest","PCIStatus","PrePCILVEF","PriorCardioShock","PreProcHgb",
            "egfr","BMI")


final.data=final.data[,colnames(final.data)%in%union(vars_model1,vars_model8)]
...
# model 1
data.selected.new=final.imp[,colnames(final.imp)%in%vars_model1]
# model 8
data.preproc.new=final.imp[,colnames(final.imp)%in%vars_model8]
